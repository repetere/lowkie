...
class lowkieSchema {
  ...
  validateDocument (_document) {
    ...
    return this.validNames.reduce((result, key) => {
      if (_document[key]) {
        ...
        if (this.flattenedScheme[`${ key }.default`] === Date.now) reslt[key] = new Date();
  ...
  populate (model_name, refs = '') {
    ...
    let model = collections.filter(collection => collection.name === model_name)[0];
    ...
    model.data = function () {
      _refs.forEach(ref => {
        ...
        let chain = toPopulate.chain();
        this.eqJoin(chain, ref, '_id');
      });
      this.data = _data;
      return this.data();
    }.bind(model);
